...
export function AnimationControls({
  ...
  return (
    <div className="flex flex-col w-full items-center mt-1 py-1 px-3 sm:py-2 sm:px-4 md:py-3 md:px-6 bg-card rounded-lg border shadow-sm">
      {/* Main controls row */}
      <div className="flex w-full flex-wrap items-center gap-2 sm:gap-4">
        <div className="flex items-center gap-1 sm:gap-2">
          <Button
            onClick={onReset}
            variant="outline"
            className="h-6.5 px-2 sm:px-3 text-xs sm:text-sm flex items-center justify-center box-border"
          >
            <SkipBack className="h-3 w-3 sm:h-4 sm:w-4 mr-1 sm:mr-2" />
            <span>Reset</span>
          </Button>
          <Button
            onClick={onPlayPause}
            variant="default"
            disabled={isLoading}
            className="h-6 px-2 sm:px-3 text-xs sm:text-sm flex items-center justify-center box-border min-w-16 sm:min-w-24"
          >
            {isPlaying ? (
              <>
      ...
      <div className="w-full">
        <div className="flex items-center justify-between">
          ...
          <Button
            ...
            className="h-6 sm:h-8 px-2 text-xs sm:text-sm"
          >
            {showSpeedControls ? (
              <>
                <ChevronUp className="h-3 w-3 sm:h-4 sm:w-4 mr-1" /> Hide
              </>
            ) : (
              <>
                <ChevronDown className="h-3 w-3 sm:h-4 sm:w-4 mr-1" /> Show
              </>
            )}
          </Button>
        </div>
        ...
        {showSpeedControls && (
          <div className="sm:mt-1 space-y-1 sm:space-y-2 sm:pt-1 border-t">
            <Slider
              value={currentSpeed}
              min={0.25}
              max={2}
